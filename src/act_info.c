...
char* const where_name[] = {
    ...
    "<worn around wrist> ", "<wielded, primary>  ", "<held>              ",
    "<wielded, second>   "};
...
void do_look(CHAR_DATA* ch, char* argument) {
  ...
  if (pexit->keyword && pexit->keyword[0] != '\0' && pexit->keyword[0] != ' ') {
    if (IS_SET(pexit->exit_info, EX_BASHED))
      act("The $d has been bashed from its hinges.", ch, NULL, pexit->keyword,
          TO_CHAR);
...
void do_score(CHAR_DATA* ch, char* argument) {
  ...
  if (GET_AC(ch) >= 101)
    ...
    strcat(buf1, "slightly armored.\r\n");
  else if (GET_AC(ch) >= 20)
    strcat(buf1, "somewhat armored.\r\n");
  else if (GET_AC(ch) >= 0)
    strcat(buf1, "armored.\r\n");
  else if (GET_AC(ch) >= -20)
    strcat(buf1, "well armored.\r\n");
  else if (GET_AC(ch) >= -40)
    strcat(buf1, "strongly armored.\r\n");
  else if (GET_AC(ch) >= -60)
    strcat(buf1, "heavily armored.\r\n");
  else if (GET_AC(ch) >= -80)
    strcat(buf1, "superbly armored.\r\n");
  else if (GET_AC(ch) >= -100)
    strcat(buf1, "divinely armored.\r\n");
  else
    strcat(buf1, "invincible!\r\n");

  if (ch->level >= 15) {
    if (IS_NPC(ch) || ch->level >= skill_table[gsn_dual].skill_level[ch->class])
      strcat(buf1, "Primary Weapon ");
    sprintf(buf, "Hitroll: %d  Damroll: %d.\r\n", get_hitroll(ch, WEAR_WIELD),
            get_damroll(ch, WEAR_WIELD));
    strcat(buf1, buf);
...
void do_time(CHAR_DATA* ch, char* argument) {
  ...
  send_to_char(buf, ch);
  sprintf(buf, "EnvyMud started up %s\rThe system time is %s\r", str_boot_time,
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (wch->level < LEVEL_IMMORTAL)
      sprintf(buf + strlen(buf), "(%2d %s %-10s) %s%s%s%s%s%s\r\n", wch->level,
              class, race_table[wch->race].name,
              IS_SET(wch->act, PLR_KILLER) ? "(KILLER) " : "",
              IS_SET(wch->act, PLR_THIEF) ? "(THIEF) " : "",
              IS_SET(wch->act, PLR_REGISTER) ? "(PK) " : "",
              IS_SET(wch->act, PLR_AFK) ? "(AFK) " : "", wch->name,
              wch->pcdata->title);
    else
...
void do_where(CHAR_DATA* ch, char* argument) {
  ...
  if (arg[0] == '\0') {
    ...
    for (victim = char_list; victim; victim = victim->next) {
      ...
      if (victim->in_room->area == ch->in_room->area && can_see(ch, victim) &&
...
void do_report(CHAR_DATA* ch, char* argument) {
  ...
  sprintf(buf, "$n reports: %d/%d hp %d/%d mana %d/%d mv %d xp.", ch->hit,
          ch->max_hit, ch->mana, ch->max_mana, ch->move, ch->max_move, ch->exp);
  ...
  if (argument[0] == '\0') {
    ...
    if (ch->pcdata->learned[sn] >= adept) {
      ...
      if (ch->pcdata->learned[sn] < adept) {
        ...
        act("You are now an adept of $T.", ch, NULL, skill_table[sn].name,
            TO_CHAR);
        act("$n is now an adept of $T.", ch, NULL, skill_table[sn].name,
            TO_ROOM);
      }
    }
  }
...
void do_channels(CHAR_DATA* ch, char* argument) {
  ...
  if (arg[0] == '\0') {
    ...
    send_to_char(!IS_SET(ch->deaf, CHANNEL_AUCTION) ? " +AUCTION" : " -auction",
                 ch);
    ...
    if (IS_HERO(ch)) {
      send_to_char(
          !IS_SET(ch->deaf, CHANNEL_IMMTALK) ? " +IMMTALK" : " -immtalk", ch);
    }

    send_to_char(!IS_SET(ch->deaf, CHANNEL_MUSIC) ? " +MUSIC" : " -music", ch);

    send_to_char(
        !IS_SET(ch->deaf, CHANNEL_QUESTION) ? " +QUESTION" : " -question", ch);
...
void do_spells(CHAR_DATA* ch, char* argument) {
  ...
  if (IS_NPC(ch) || (!IS_NPC(ch) && !class_table[ch->class].fMana)) {
