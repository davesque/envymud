...
void move_char(CHAR_DATA* ch, int door) {
  ...
  if (IS_SET(pexit->exit_info, EX_CLOSED)) {
    ...
    if (IS_SET(pexit->exit_info, EX_PASSPROOF)) {
      act("You are unable to pass through the $d.  Ouch!", ch, NULL,
...
void do_close(CHAR_DATA* ch, char* argument) {
  ...
  if ((door = find_door(ch, arg)) >= 0) {
    ...
    if (IS_SET(pexit->exit_info, EX_BASHED)) {
      act("The $d has been bashed open and cannot be closed.", ch, NULL,
          pexit->keyword, TO_CHAR);
      return;
    }
...
void do_bash(CHAR_DATA* ch, char* argument) {
  ...
  if ((door = find_door(ch, arg)) >= 0) {
    ...
    if (IS_SET(pexit->exit_info, EX_BASHPROOF)) {
      act("WHAAAAM!!!  You bash against the $d, but it doesn't budge.", ch,
          NULL, pexit->keyword, TO_CHAR);
      act("WHAAAAM!!!  $n bashes against the $d, but it holds strong.", ch,
    ...
    if ((get_curr_str(ch) >= 20) &&
      ...
      act("OW!  You bash against the $d, but it doesn't budge.", ch, NULL,
          pexit->keyword, TO_CHAR);
      act("$n bashes against the $d, but it holds strong.", ch, NULL,
  ...
  for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
    if (!gch->deleted && gch != ch && IS_AWAKE(gch) && can_see(gch, ch) &&
        !gch->fighting && (IS_NPC(gch) && !IS_AFFECTED(gch, AFF_CHARM)) &&
...
void do_snare(CHAR_DATA* ch, char* argument) {
  ...
  if (arg[0] == '\0') {
    ...
    if ((IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM)) ||
        (!IS_NPC(ch) && number_percent() < ch->pcdata->learned[gsn_snare])) {
      ...
      act("You failed to ensnare $M.  Uh oh!", ch, NULL, victim, TO_CHAR);
      act("$n tried to ensnare you!  Get $m!", ch, NULL, victim, TO_VICT);
      act("$n attempted to ensnare $N, but failed!", ch, NULL, victim,
          TO_NOTVICT);
    }
...
void do_untangle(CHAR_DATA* ch, char* argument) {
  ...
  if ((IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM)) ||
      (!IS_NPC(ch) && number_percent() < ch->pcdata->learned[gsn_untangle])) {
    affect_strip(victim, gsn_snare);
...
void game_u_l_t(CHAR_DATA* ch, CHAR_DATA* croupier, char* argument) {
  ...
  if (amount > atoi(limit)) {
    act("$n tells you, 'Sorry, the house limit is $t.'", croupier, limit, ch,
        TO_VICT);
    ch->reply = croupier;
    return;
  }
   ...
   */
  act("You place $t gold coins on the table, and bet '$T'.", ch, wager, choice,
      TO_CHAR);
  ...
  sprintf(msg, "$n rolls the dice: they come up %d, %d, and %d", die1, die2,
          die3);
  ...
  if (die1 == die2 && die2 == die3) {
    ...
    if (ichoice == 3) {
      ...
      act("It's a TRIPLE!  You win $t gold coins!", ch, haul, NULL, TO_CHAR);
      ch->gold += amount;
    } else
