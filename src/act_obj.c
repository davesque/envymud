...
void get_obj(CHAR_DATA* ch, OBJ_DATA* obj, OBJ_DATA* container) {
  ...
  if (IS_OBJ_STAT(obj, ITEM_HOLY)) {
    act("You are burned by HOLY FIRE from $p.  Ouch!", ch, obj, NULL, TO_CHAR);
...
void do_put(CHAR_DATA* ch, char* argument) {
  ...
  if (str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
    ...
    if (get_obj_weight(obj) + get_obj_weight(container) - container->weight >
        container->value[0]) {
...
void do_give(CHAR_DATA* ch, char* argument) {
  ...
  if ((IS_OBJ_STAT(obj, ITEM_HOLY) && victim->race == race_lookup("Vampire")) ||
    ...
    act("$n tries to give $N a $p but $E refuses.", ch, obj, victim, TO_ROOM);
    act("You refuse to accept the $p from $n.", ch, obj, victim, TO_VICT);
    return;
  }
...
void do_drink(CHAR_DATA* ch, char* argument) {
  ...
  switch (obj->item_type) {
    ...
    case ITEM_DRINK_CON:
      ...
      if (obj->value[3] != 0 && (ch->race != race_lookup("Vampire") &&
...
void do_eat(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_NPC(ch) || (IS_NPC(ch) && is_affected(ch, gsn_charm_person)))
    extract_obj(obj);
  return;
}
...
void do_wear(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_NPC(ch) && IS_AFFECTED(ch, AFF_GHOUL)) {
    send_to_char("You may not wear, wield, or hold anything in ghoul form.\r\n",
                 ch);
    return;
  }
...
void do_sacrifice(CHAR_DATA* ch, char* argument) {
  ...
  if (arg[0] == '\0' || !str_cmp(arg, ch->name)) {
    ...
    act("$n offers $mself to God, who graciously declines.", ch, NULL, NULL,
        TO_ROOM);
    return;
  }
...
void do_recite(CHAR_DATA* ch, char* argument) {
  ...
  if ((IS_NPC(ch) && IS_SET(ch->act, ACT_PET)) || IS_AFFECTED(ch, AFF_CHARM)) {
    act("You try to recite $p, but you have no free will.", ch, scroll, NULL,
        TO_CHAR);
    act("$n tries to recite $p, but has no free will.", ch, scroll, NULL,
        TO_ROOM);
    return;
  }
  ...
  if (!IS_NPC(ch) && !(number_percent() < ch->pcdata->learned[gsn_scrolls])) {
    switch (number_bits(3)) {
      ...
      case 2:
      case 3:
        act("You can't understand $p at all.", ch, scroll, NULL, TO_CHAR);
        act("$n can't understand $p at all.", ch, scroll, NULL, TO_ROOM);
        return;
      case 4:
      case 5:
      case 6:
        send_to_char("You must have said something incorrectly.\r\n", ch);
        act("$n must have said something incorrectly.", ch, NULL, NULL,
            TO_ROOM);
        act("$p blazes brightly, then is gone.", ch, scroll, NULL, TO_CHAR);
        act("$p blazes brightly and disappears.", ch, scroll, NULL, TO_ROOM);
        extract_obj(scroll);
        return;
      case 7:
        act("You completely botch the recitation, and $p bursts into flames!!",
            ch, scroll, NULL, TO_CHAR);
        act("$p glows and then bursts into flame!", ch, scroll, NULL, TO_ROOM);
        /*
...
void do_brandish(CHAR_DATA* ch, char* argument) {
  ...
  if ((IS_NPC(ch) && IS_SET(ch->act, ACT_PET)) || IS_AFFECTED(ch, AFF_CHARM)) {
    act("You try to brandish $p, but you have no free will.", ch, staff, NULL,
        TO_CHAR);
    act("$n tries to brandish $p, but has no free will.", ch, staff, NULL,
        TO_ROOM);
    return;
  }
  ...
  if (staff->value[2] > 0) {
    ...
    if (!IS_NPC(ch) && !(number_percent() < ch->pcdata->learned[gsn_staves])) {
      switch (number_bits(3)) {
        ...
        case 2:
        case 3:
          act("You are unable to invoke the power of $p.", ch, staff, NULL,
              TO_CHAR);
          act("$n is unable to invoke the power of $p.", ch, staff, NULL,
              TO_ROOM);
          return;
        case 4:
        case 5:
        case 6:
          act("You summon the power of $p, but it fizzles away.", ch, staff,
              NULL, TO_CHAR);
          act("$n summons the power of $p, but it fizzles away.", ch, staff,
              NULL, TO_ROOM);
          if (--staff->value[2] <= 0) {
          ...
          return;
        case 7:
          act("You can't control the power of $p, and it shatters!", ch, staff,
              NULL, TO_CHAR);
          act("$p shatters into tiny pieces!", ch, staff, NULL, TO_ROOM);
          /*
           * damage( ) call after extract_obj in case the damage would
           * have extracted ch.  This is okay because we merely mark
           * obj->deleted; it still retains all values until list_update.
...
void do_zap(CHAR_DATA* ch, char* argument) {
  ...
  if ((IS_NPC(ch) && IS_SET(ch->act, ACT_PET)) || IS_AFFECTED(ch, AFF_CHARM)) {
    act("You try to zap $p, but you have no free will.", ch, wand, NULL,
        TO_CHAR);
    act("$n tries to zap $p, but has no free will.", ch, wand, NULL, TO_ROOM);
    return;
  }
  ...
  if (wand->value[2] > 0) {
    ...
    if (!IS_NPC(ch) && !(number_percent() < ch->pcdata->learned[gsn_wands])) {
      switch (number_bits(3)) {
        ...
        case 2:
        case 3:
          act("You are unable to invoke the power of $p.", ch, wand, NULL,
              TO_CHAR);
          act("$n is unable to invoke the power of $p.", ch, wand, NULL,
              TO_ROOM);
          return;
        case 4:
        case 5:
        case 6:
          act("You summon the power of $p, but it fizzles away.", ch, wand,
              NULL, TO_CHAR);
          act("$n summons the power of $p, but it fizzles away.", ch, wand,
              NULL, TO_ROOM);
          if (--wand->value[2] <= 0) {
          ...
          return;
        case 7:
          act("You can't control the power of $p, and it explodes!", ch, wand,
              NULL, TO_CHAR);
...
void do_buy(CHAR_DATA* ch, char* argument) {
  ...
  if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
    ...
    if ((item_count > 1) && !IS_SET(obj->extra_flags, ITEM_INVENTORY)) {
      act("$n tells you, 'Sorry - $p is something I have only one of'", keeper,
          obj, ch, TO_CHAR);
      ch->reply = keeper;
      return;
    }
  ...
  if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
    ...
    for (obj = keeper->carrying; obj; obj = obj->next_content) {
      ...
      if (can_see_obj(ch, obj) &&
          (arg[0] == '\0' || !str_cmp(arg, "all") || is_name(arg, obj->name))) {
        if (!found) {
          found = TRUE;
...
void do_sell(CHAR_DATA* ch, char* argument) {
  ...
  if ((cost = get_cost(keeper, obj, FALSE)) <= 0 || obj->level > LEVEL_HERO) {
    act("$n looks uninterested in $p.", keeper, obj, ch, TO_VICT);
    return;
  }

  if (IS_SET(obj->extra_flags, ITEM_POISONED)) {
    act("$n tells you 'I won't buy that!  It's poisoned!'", keeper, NULL, ch,
        TO_VICT);
    ch->reply = keeper;
    return;
  }
...
void do_value(CHAR_DATA* ch, char* argument) {
  ...
  if (!can_see_obj(keeper, obj)) {
    act("$n tells you 'You are offering me an imaginary object!?!?'.", keeper,
        NULL, ch, TO_VICT);
    ch->reply = keeper;
    return;
  }
...
void do_poison_weapon(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_NPC(ch) &&
      (get_curr_dex(ch) < 20 || ch->pcdata->condition[COND_DRUNK] > 0)) {
    send_to_char(
  ...
  act("You mix $p in $P, creating a deadly poison!", ch, pobj, wobj, TO_CHAR);
  act("$n mixes $p in $P, creating a deadly poison!", ch, pobj, wobj, TO_ROOM);
  act("You pour the poison over $p, which glistens wickedly!", ch, obj, NULL,
      TO_CHAR);
  act("$n pours the poison over $p, which glistens wickedly!", ch, obj, NULL,
      TO_ROOM);
...
void do_donate(CHAR_DATA* ch, char* arg) {
  ...
  if (str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
    ...
    act("You send $p flying to the $P.", ch, obj, container, TO_CHAR);
    send_to_room("A loud clank is heard from the pit!", container->in_room);
  } else {
    for (obj = ch->carrying; obj; obj = obj_next) {
      obj_next = obj->next_content;
