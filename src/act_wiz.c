...
void do_goto(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_SET(ch->act, PLR_WIZINVIS)) {
    act("$n $T.", ch, NULL, (ch->pcdata && ch->pcdata->bamfout[0] != '\0')
                                ? ch->pcdata->bamfout
  ...
  if (!IS_SET(ch->act, PLR_WIZINVIS)) {
    act("$n $T.", ch, NULL, (ch->pcdata && ch->pcdata->bamfin[0] != '\0')
                                ? ch->pcdata->bamfin
...
void do_rstat(CHAR_DATA* ch, char* argument) {
  ...
  for (door = 0; door <= 5; door++) {
    ...
    if ((pexit = location->exit[door])) {
      ...
      strcat(buf1, buf);
      sprintf(
          buf, "Keyword: '%s'.  Description: %s", pexit->keyword,
...
void do_ostat(CHAR_DATA* ch, char* argument) {
  ...
  sprintf(buf,
          ...
          !obj->carried_by ? "(none)" : obj->carried_by->name, obj->wear_loc);
  strcat(buf1, buf);
...
void do_mstat(CHAR_DATA* ch, char* argument) {
  ...
  sprintf(buf, "Vnum: %d.  Sex: %s.  Room: %d.\r\n",
          ...
          victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE
                                                 ? "female"
                                                 : "neuter",
...
void do_mwhere(CHAR_DATA* ch, char* argument) {
  ...
  for (victim = char_list; victim; victim = victim->next) {
    if (IS_NPC(victim) && victim->in_room && is_name(arg, victim->name)) {
      found = TRUE;
      sprintf(buf, "[%5d] %-28s [%5d] %s\r\n", victim->pIndexData->vnum,
              victim->short_descr, victim->in_room->vnum,
              victim->in_room->name);
      strcat(buf1, buf);
    }
  }
  ...
  if (fAll) {
    ...
    for (sn = 0; sn < MAX_SKILL; sn++) {
      if (skill_table[sn].name) {
        ...
        else if (skill_table[sn].skill_level[victim->class] <=
                 get_trust(victim))
...
void do_mset(CHAR_DATA* ch, char* argument) {
  ...
  if (!str_cmp(arg2, "race")) {
    ...
    if ((wield = get_eq_char(victim, WEAR_WIELD)) &&
        !IS_SET(race_table[victim->race].race_abilities, RACE_WEAPON_WIELD)) {
