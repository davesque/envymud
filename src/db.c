...
void load_resets(FILE* fp) {
  ...
  for (;;) {
    ...
    pReset->arg3 =
        (letter == 'G' || letter == 'R') ? 0 : fread_number(fp, &stat);
    fread_to_eol(fp);
    ...
    for (;;) {
      ...
      if (letter == 'D') {
        ...
        switch (locks) {
          ...
          case 8:
            pexit->exit_info =
                EX_ISDOOR | EX_PICKPROOF | EX_BASHPROOF | EX_PASSPROOF;
            break;
        }
    ...
    for (pRoomIndex = room_index_hash[iHash]; pRoomIndex;
      ...
      for (door = 0; door <= 5; door++) {
        if ((pexit = pRoomIndex->exit[door]) && (to_room = pexit->to_room) &&
          ...
          sprintf(buf, "Fix_exits: %d:%d -> %d:%d -> %d.", pRoomIndex->vnum,
  ...
  for (pReset = pArea->reset_first; pReset; pReset = pReset->next) {
    ...
    switch (pReset->command) {
      ...
      case 'P':
        ...
        if (pArea->nplayer > 0 || !(obj_to = get_obj_type(pObjToIndex)) ||
...
CHAR_DATA* create_mobile(MOB_INDEX_DATA* pMobIndex) {
  ...
  mob->gold = number_fuzzy(10) * number_fuzzy(pMobIndex->level) *
