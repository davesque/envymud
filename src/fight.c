...
void dam_message
args((CHAR_DATA * ch, CHAR_DATA* victim, int dam, int dt, int wpn));
...
void violence_update(void) {
  ...
  for (ch = char_list; ch; ch = ch->next) {
    ...
    if ((victim = ch->fighting)) {
      if (IS_AWAKE(ch) && ch->in_room == victim->in_room && !victim->deleted) {
    ...
    for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
      ...
      if (!IS_NPC(ch) && (!IS_NPC(rch) || IS_AFFECTED(rch, AFF_CHARM)) &&
          is_same_group(ch, rch) && IS_NPC(victim))
        break;
...
void one_hit(CHAR_DATA* ch, CHAR_DATA* victim, int dt, int wpn) {
   ...
   */
  race_hit_bonus =
      (race_table[victim->race].size - race_table[ch->race].size) / 3;
  ...
  thac0 = interpolate(ch->level, thac0_00, thac0_47) - get_hitroll(ch, wpn) -
          race_hit_bonus;
  ...
  if (IS_NPC(ch)) {
    ...
    if (wield && dam > 1000) {
      sprintf(buf, "One_hit dam range > 1000 from %d to %d", wield->value[1],
              wield->value[2]);
      bug(buf, 0);
    }
  }
...
void damage(CHAR_DATA* ch, CHAR_DATA* victim, int dam, int dt, int wpn) {
  ...
  if (dam > 1000) {
    ...
    if (IS_NPC(ch) && ch->desc)
      sprintf(buf, "Damage: %d from %s by %s: > 1000 points with %d dt!", dam,
              ch->name, ch->desc->original->name, dt);
    else
      sprintf(buf, "Damage: %d from %s: > 1000 points with %d dt!", dam,
              ch->name, dt);
  ...
  if (victim != ch) {
    ...
    if (victim->position > POS_STUNNED) {
      ...
      if (IS_NPC(ch) && IS_NPC(victim) && victim->master &&
          victim->master->in_room == ch->in_room && number_bits(2) == 0) {
        stop_fighting(ch, FALSE);
    ...
    if (dt >= TYPE_HIT || dt == gsn_kick) {
      ...
      if (IS_NPC(ch) &&
          ...
          number_percent() < UMIN(25, UMAX(10, ch->level)) && !IS_NPC(victim))
        use_magical_item(ch);
  ...
  if ((dam > 1) && IS_AFFECTED(victim, AFF_FLAMING) && victim != ch) {
    ...
    (*skill_table[sntemp].spell_fun)(sntemp, (victim->level / 4), victim, ch);
  }

  if (is_affected(victim, gsn_berserk) && victim->position <= POS_STUNNED)
    affect_strip(victim, gsn_berserk);

  if (dam > 0 && dt > TYPE_HIT && is_wielding_poisoned(ch, wpn) &&
    ...
    if (!IS_NPC(victim)) {
      ...
      if (IS_NPC(ch)) {
        if (victim->exp > EXP_PER_LEVEL * (victim->level - 1))
          gain_exp(victim,
                   (EXP_PER_LEVEL * (victim->level - 1) - victim->exp) / 4);
      } else {
        if (ch != victim) {
          if (IS_SET(victim->act, PLR_REGISTER)) {
            ...
            if (victim->exp < 1001) {
              send_to_char("You no longer have any exps to give.\r\n", victim);
...
bool is_safe(CHAR_DATA* ch, CHAR_DATA* victim) {
  if (!IS_NPC(ch) && IS_AFFECTED(ch, AFF_GHOUL)) {
    send_to_char("You may not participate in combat while in ghoul form.\r\n",
                 ch);
    return TRUE;
  }
  ...
  if (ch->level < 16 && !ch->fighting) {
    send_to_char("You may not pkill or psteal with or without registering.\r\n",
                 ch);
    return TRUE;
  }

  if (victim->level >= LEVEL_HERO) {
    act("$N is a HERO and is automatically safe.", ch, NULL, victim, TO_CHAR);
    return TRUE;
  }
...
bool is_wielding_poisoned(CHAR_DATA* ch, int wpn) {
  ...
  if ((obj = get_eq_char(ch, wpn)) && IS_SET(obj->extra_flags, ITEM_POISONED))
    return TRUE;
...
void death_cry(CHAR_DATA* ch) {
  ...
  for (door = 0; door <= 5; door++) {
    ...
    if ((pexit = was_in_room->exit[door]) && pexit->to_room &&
...
void group_gain(CHAR_DATA* ch, CHAR_DATA* victim) {
  ...
  for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
    ...
    if (!str_infix(race_table[lch->race].name, race_table[gch->race].hate) &&
        members > 1) {
      send_to_char(
          "You lost a third of your exps due to grouping with scum.\r\n", ch);
      xp -= xp / 3;
    }
...
int xp_compute(CHAR_DATA* gch, CHAR_DATA* victim) {
  ...
  if (!str_infix(race_table[victim->race].name, race_table[gch->race].hate)) {
    bonus += (float)1 / 10;
  }
...
void do_circle(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_NPC(ch) &&
    ...
    send_to_char("You'd better leave the assassin trade to thieves.\r\n", ch);
    return;
  }
  ...
  if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_circle] / 2) {
    stop_fighting(victim, FALSE);
...
void do_flee(CHAR_DATA* ch, char* argument) {
  ...
  if (IS_AFFECTED(ch, AFF_HOLD)) {
    ...
    act("$n wants to flee, but is caught in a snare!", ch, NULL, NULL, TO_ROOM);
    return;
  }
...
void do_berserk(CHAR_DATA* ch, char* argument) {
  ...
  send_to_char("Your weapon hits your foe and blood splatters all over!\r\n",
               ch);
...
void do_rescue(CHAR_DATA* ch, char* argument) {
  ...
  if (!fch ||
      (!IS_NPC(ch) && number_percent() > ch->pcdata->learned[gsn_rescue])) {
...
void do_kick(CHAR_DATA* ch, char* argument) {
  ...
  if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_kick])
    damage(ch, victim, number_range(1, ch->level), gsn_kick, WEAR_NONE);
  else
...
void do_disarm(CHAR_DATA* ch, char* argument) {
  ...
  if (!get_eq_char(victim, WEAR_WIELD) && !get_eq_char(victim, WEAR_WIELD_2)) {
...
void pc_screech(CHAR_DATA* ch) {
  ...
  for (victim = ch->in_room->people; victim; victim = victim_next) {
    ...
    act("Your ears pop from $n's scream.  Ouch!", ch, NULL, victim, TO_VICT);
...
void pc_spit(CHAR_DATA* ch) {
  ...
  for (victim = ch->in_room->people; victim; victim = victim_next) {
    ...
    damage(ch, victim, number_range(1, ch->level), gsn_poison_weapon,
           WEAR_NONE);
  }
...
void do_fee(CHAR_DATA* ch, char* argument) {
  send_to_char("IF you wish to feed on someone or something, type FEED!\r\n",
               ch);
  return;
}
...
void do_feed(CHAR_DATA* ch, char* argument) {
  ...
  if (!IS_NPC(ch) && number_percent() < (34 - victim->level + ch->level)) {
  ...
  if (saves_spell(ch->level, victim) || number_bits(1) == 0)
    return;
...
void do_stake(CHAR_DATA* ch, char* argument) {
  ...
  if (!found) {
    ...
    act("$n screams and lunges at $N then realizes $e doesnt have a stake", ch,
        NULL, victim, TO_ROOM);
    multi_hit(ch, victim, TYPE_UNDEFINED);
    return;
  }
...
void use_magical_item(CHAR_DATA* ch) {
  ...
  for (obj = ch->carrying; obj; obj = obj->next_content) {
    if ((obj->item_type == ITEM_SCROLL || obj->item_type == ITEM_WAND ||
