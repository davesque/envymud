...
int get_hitroll(CHAR_DATA* ch, int wpn) {
  ...
  if (wpn == WEAR_WIELD)
    ...
    sprintf(buf, "get_hitroll: Invalid weapon location %d on %s.", wpn,
            ch->name);
    bug(buf, 0);
    return 0;
  }
...
int get_damroll(CHAR_DATA* ch, int wpn) {
  ...
  if (wpn == WEAR_WIELD)
    ...
    char buf[MAX_STRING_LENGTH];
    sprintf(buf, "get_damroll: Invalid weapon location %d on %s.", wpn,
            ch->name);
    bug(buf, 0);
    return 0;
  }
...
void affect_modify(CHAR_DATA* ch, AFFECT_DATA* paf, bool fAdd) {
  ...
  switch (paf->location) {
    default:
      sprintf(buf, "Affect_modify: unknown location %d on %s.", paf->location,
              ch->name);
      bug(buf, 0);
      return;
...
void unequip_char(CHAR_DATA* ch, OBJ_DATA* obj) {
  ...
  if (obj->wear_loc == WEAR_NONE) {
    sprintf(buf, "Unequip_char: %s already unequipped with %d.", ch->name,
            obj->pIndexData->vnum);
    bug(buf, 0);
    return;
  }
...
char* item_type_name(OBJ_DATA* obj) {
  ...
  if (in_obj->carried_by)
    ...
    sprintf(buf, "Item_type_name: unknown type %d from %s owned by (unknown).",
