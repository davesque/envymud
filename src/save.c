...
void fwrite_char(CHAR_DATA* ch, FILE* fp) {
  ...
  if (IS_NPC(ch)) {
    ...
    fprintf(fp, "AtrPrm      %d %d %d %d %d\n", ch->pcdata->perm_str,
            ch->pcdata->perm_int, ch->pcdata->perm_wis, ch->pcdata->perm_dex,
            ch->pcdata->perm_con);
  ...
  if ((fp = fopen(strsave, "r"))) {
    ...
    for (;;) {
      ...
      if (!str_cmp(word, "PLAYER")) {
        if (fread_char(ch, fp)) {
          sprintf(buf, "Load_char_obj:  %s section PLAYER corrupt.\r\n", name);
          bug(buf, 0);
        ...
        if (!fread_obj(ch, fp)) {
          sprintf(buf, "Load_char_obj:  %s section OBJECT corrupt.\r\n", name);
          bug(buf, 0);
...
int fread_char(CHAR_DATA* ch, FILE* fp) {
  ...
  struct key_data key_tab[] = {
      ...
      {"AtrPrm",
       ...
       {&ch->pcdata->perm_str, &ch->pcdata->perm_int, &ch->pcdata->perm_wis,
        &ch->pcdata->perm_dex, &ch->pcdata->perm_con, NULL}},
      {"AtrMd",
       FALSE,
       MAND,
       {&ch->pcdata->mod_str, &ch->pcdata->mod_int, &ch->pcdata->mod_wis,
        &ch->pcdata->mod_dex, &ch->pcdata->mod_con, NULL}},
      {"Cond",
       FALSE,
       DEFLT,
       {&ch->pcdata->condition[0], &ch->pcdata->condition[1],
        &ch->pcdata->condition[2], NULL}},
  ...
  for (; !feof(fp);) {
    ...
    for (i = last_key;
         i < last_key + num_keys && str_cmp(key_tab[i % num_keys].key, word);)
      i++;
    ...
    if (*key_tab[i].key) /* Key entry found in key_tab */
    {
      ...
      paf->type = affect_lookup(fread_string(fp, &status));
  ...
  for (i = 0; *key_tab[i].key; i++) {
    ...
    if (key_tab[i].deflt == MAND) {
      sprintf(buf, "Manditory field '%s' missing from pfile.", key_tab[i].key);
      bug(buf, 0);
      error_count++;
      continue;
    }
...
void recover(FILE* fp, long fpos) {
  ...
  while (!feof(fp)) {
    ...
    if (!strncmp(buf, "#OBJECT", 7) || !strncmp(buf, "#END", 4)) {
      fseek(fp, fpos, 0);
      return;
    }
  }
...
int fread_obj(CHAR_DATA* ch, FILE* fp) {
  ...
  struct key_data key_tab[] = {
      ...
      {"Values",
       FALSE,
       MAND,
       {&obj.value[0], &obj.value[1], &obj.value[2], &obj.value[3], NULL}},
      {"\0", FALSE, 0}};
