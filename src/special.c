...
SPEC_FUN* spec_lookup(const char* name) {
  ...
  if (!str_cmp(name, "spec_cast_ghost"))
    return spec_cast_ghost;
  if (!str_cmp(name, "spec_cast_judge"))
    return spec_cast_judge;
  if (!str_cmp(name, "spec_cast_mage"))
    return spec_cast_mage;
  if (!str_cmp(name, "spec_cast_psionicist"))
    return spec_cast_psionicist;
  ...
  if (!str_cmp(name, "spec_poison"))
    return spec_poison;
  if (!str_cmp(name, "spec_repairman"))
    return spec_repairman;
...
bool spec_poison(CHAR_DATA* ch) {
  ...
  if (ch->position != POS_FIGHTING || !(victim = ch->fighting) ||
...
bool spec_thief(CHAR_DATA* ch) {
  ...
  for (victim = ch->in_room->people; victim; victim = victim->next_in_room) {
    if (IS_NPC(victim) || victim->level >= LEVEL_IMMORTAL ||
        number_bits(3) != 0 || !can_see(ch, victim)) /* Thx Glop */
      continue;
...
bool spec_cast_ghost(CHAR_DATA* ch) {
  ...
  if (weather_info.sunlight != SUN_DARK) {
    ...
    act("A beam of sunlight strikes $n, destroying $m.", ch, NULL, NULL,
        TO_ROOM);
  ...
  if (IS_SET(pexit->exit_info, EX_BASHED)) {
    ...
    if ((to_room = pexit->to_room) &&
      ...
      for (rch = to_room->people; rch; rch = rch->next_in_room)
        act("The $d is set back on its hinges.", rch, NULL, pexit_rev->keyword,
            TO_CHAR);
    }
