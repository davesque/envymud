...
void demote_level(CHAR_DATA* ch) {
  ...
  ch->level -= 1;
  sprintf(buf, "Your loss is: %d/%d hp, %d/%d m, %d/%d mv %d/%d prac.\r\n",
          add_hp, ch->max_hit, add_mana, ch->max_mana, add_move, ch->max_move,
          add_prac, ch->practice);
  send_to_char(buf, ch);
  return;
...
int hit_gain(CHAR_DATA* ch) {
  int gain;
  int racehpgainextra =
      (ch->max_hit * race_table[ch->race].hp_gain) / ch->max_hit;
...
int mana_gain(CHAR_DATA* ch) {
  int gain;
  int racemanaextra =
      (ch->max_mana * race_table[ch->race].mana_gain) / ch->max_mana;
...
int move_gain(CHAR_DATA* ch) {
  int gain;
  int racemoveextra =
      (ch->max_move * race_table[ch->race].move_gain) / ch->max_move;
...
void mobile_update(void) {
  ...
  for (ch = char_list; ch; ch = ch->next) {
    ...
    /* Scavenge */
    if (IS_SET(ch->act, ACT_SCAVENGER) && ch->in_room->contents &&
        number_bits(2) == 0) {
      OBJ_DATA* obj;
      OBJ_DATA* obj_best;
      int max;
    ...
    if (!IS_SET(ch->act, ACT_SENTINEL) && (door = number_bits(rnum)) <= 5 &&
...
void char_update(void) {
  ...
  for (ch = char_list; ch; ch = ch->next) {
    ...
    if (!IS_NPC(ch) &&
        (ch->level < LEVEL_IMMORTAL || (!ch->desc && !IS_SWITCHED(ch)))) {
      OBJ_DATA* obj;
...
void aggr_update(void) {
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (d->connected != CON_PLAYING || ch->level >= LEVEL_IMMORTAL ||
        !ch->in_room)
      continue;
    ...
    for (mch = ch->in_room->people; mch; mch = mch->next_in_room) {
      ...
      if (!IS_NPC(mch) || mch->deleted || mch->fighting ||
          IS_AFFECTED(mch, AFF_CHARM) || !IS_AWAKE(mch) ||
          (IS_SET(mch->act, ACT_WIMPY) && IS_AWAKE(ch)) || !can_see(mch, ch) ||
      ...
      if (!str_infix(race_table[ch->race].name, race_table[mch->race].hate))
        hate = TRUE;
...
void update_handler(void) {
  ...
  if (--pulse_mobile <= 0) {
    pulse_mobile = number_range(PULSE_MOBILE / 2, 3 * PULSE_MOBILE / 2);
    mobile_update();
  }
